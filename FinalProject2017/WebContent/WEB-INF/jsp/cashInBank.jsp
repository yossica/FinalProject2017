...
<script type="text/javascript">
	...
	function filter() {
		...
		if (filterStartDate == "" && filterEndDate == "") {
			...
			var startDate = new Date(parseInt(s[0]), parseInt(s[1]) - 1,
					parseInt(s[2]));
			var e = filterEndDate.split("-");
			var endDate = new Date(parseInt(e[0]), parseInt(e[1]) - 1,
					parseInt(e[2]));
			if (startDate > endDate) {
				//document.getElementById("errorMessage").innerHTML = "Start Date must be later than End Date!";
				sweetAlert("Oops...",
						"Start Date must be later than End Date!", "error");
			} else {
				flyToPage("filter");
			}
		} else {
			...
			sweetAlert(
					"Oops...",
					"Start Date and End Date must be either both filled or emptied!",
					"error");
		}
	}
	function balance() {
	...
	<html:form action="/cashInBank" method="post">
		...
		<div id="page-wrapper">
			...
			<div class="row" style="margin-top: 1%;">
				<div class="col-lg-12">
					<label>Remaining Balance: <bean:write name="cashInBankForm"
							property="remainingBalance" format="IDR #,###.##" /></label>
					<div class="pull-right">
						...
						<button type="button" class="btn btn-primary"
							onclick="javascript:balance()">Balancing</button>
					</div>
					<br /> <br />
				</div>
			</div>
					...
					<div class="row">
						...
						<div class="col-md-4">
							<input type="date" class="form-control" style="width: 100%;"
								...
								value="<bean:write property="filterStartDate" name="cashInBankForm" />" />
						</div>
						<div class="col-md-2">To</div>
						<div class="col-md-4">
							<input type="date" class="form-control" style="width: 100%;"
								name="filterEndDate"
								value="<bean:write property="filterEndDate" name="cashInBankForm" />" />
						</div>
					</div>
				</div>
				<div class="col-md-12"
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="panel-body">
						<div id="listCashInBank" class="table-responsive"
							...
							<table class="table table-hover">
								...
								<tbody>
									<logic:notEmpty property="transactionList"
										name="cashInBankForm">
										<logic:iterate id="cashInBank" property="transactionList"
											name="cashInBankForm">
											<tr>
												<td><bean:write property="transactionDate"
														name="cashInBank" /></td>
												<td><bean:write property="cashFlowCategoryName"
														name="cashInBank" /></td>
												<td><bean:write property="description"
														name="cashInBank" /></td>
												<logic:equal value="1" name="cashInBank" property="isDebit">
													<td><bean:write property="amount" name="cashInBank"
															format="#,###.##" /></td>
													<td></td>
												</logic:equal>
												...
												<td><bean:write property="balance" name="cashInBank"
														format="#,###.##" /></td>
											</tr>
										</logic:iterate>
									</logic:notEmpty>
									<logic:empty property="transactionList" name="cashInBankForm">
										<tr>
											<td colspan="6" align="center">There are no data based
												on this filter</td>
										</tr>
									</logic:empty>
								</tbody>
						...
						<div class="pull-right" style="margin-top: 15px;">
							<button type="button" class="btn btn-primary"
								onclick="javascript:flyToPage('transfer')">Transfer to
								Petty Cash</button>
							<button type="button" class="btn btn-primary"
								onclick="javascript:flyToPage('debit')">Debit</button>
							<button type="button" class="btn btn-primary"
								onclick="javascript:flyToPage('credit')">Credit</button>
						</div>
					</div>
				</div>
			</div>
